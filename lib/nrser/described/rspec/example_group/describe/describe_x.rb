...
# The core, mostly internal method that all RSpec's description methods lead
...
# if you want the RSpec functionality but absolutely have to set some
# metadata key that we use for something else.
# 
# @param [Array] description
#   Optional list of elements that compose the custom description.
#   
#   Will be passed to {NRSER::RSpec::Format.description} to produce the
#   string value that is in turn passed to {RSpec.describe}.
# 
# @param [Symbol] type
#   The RSpec "type" of the example group, which is used to determine the
...
def describe_x  *description,
  ...
  describe(
    # NRSER::RSpec::Format.description( *description, type: type ),
    x_description,
    **metadata,
